...
```
modules/13-my-new-module.md
```

Use the same structure as existing modules:

```markdown
# Module 13: Your Title
...
**Step 2:** Add one entry to `manifest.json`:

```json
{
  "id": "13-my-new-module",
  "title": "Your Title",
  "file": "modules/13-my-new-module.md",
  "estimatedMinutes": 10
}
```

That's it. No code changes. OpenCode will pick it up automatically via `AGENTS.md`.
...
```
exercises/08-my-exercise.md
```

Structure:

```markdown
# Exercise 8: Your Title
...
To connect an exercise to a module, the facilitator currently uses the module number to find the matching exercise. If you want to make the connection explicit, you can add an `exercise` field to the module's `manifest.json` entry (e.g., `"exercise": "exercises/08-my-exercise.md"`).

---

## Editing the Facilitator Behavior

All facilitator behavior is controlled by `AGENTS.md`. To change how OpenCode behaves:
...
Changes to `AGENTS.md` take effect immediately on next session open — no restart needed.
...
Edit these directly. The facilitator reads them at session start.
...
- [ ] Fill in `org/approved-mcps.json` (or remove entries if none are approved yet)
...
4. Learners run `npm run setup`, then open in OpenCode
